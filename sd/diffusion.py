...
class TimeEmbedding(nn.Module):
    ...
    def forward(self, timestep: torch.Tensor) -> torch.Tensor:
        # timestep: (1, embedding_dim: 320)
        x = self.linear_1(timestep)
        x = F.silu(x)
        x = self.linear_2(x)

        # (1, embedding_dim * 4: 1280)
        return x
...
class UNET_ResidualBlock(nn.Module):
    ...
    def forward(self, feature: torch.Tensor, time: torch.Tensor) -> torch.Tensor:
        ...
        # Broadcast time embedding over spatial dims: (B, C) → (B, C, 1, 1)
...
class UNET_AttentionBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor, context: torch.Tensor) -> torch.Tensor:
        ...
        # (Batch_Size, Height * Width, Features * 4) * (Batch_Size, Height * Width, Features * 4)
